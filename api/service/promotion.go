...
func (s *Promotion) CreateRule(ctx context.Context, input *model.CreatePromotionRuleInput) (*db_model.PromotionRule, error) {
	...
	err := s.db.Transaction(func(tx *gorm.DB) error {
		...
		var err error
		rule, err = s.promotionRepository.Save(ctx, tx, rule)
		if err != nil {
...
func (s *Promotion) UpdateRule(ctx context.Context, id string, input *model.UpdatePromotionRuleInput) (*db_model.PromotionRule, error) {
	...
	err := s.db.Transaction(func(tx *gorm.DB) error {
		var err error
		rule, err = s.promotionRepository.Get(ctx, tx, id)
		if err != nil {
			return err
		}
...
func (s *Promotion) DeleteRule(ctx context.Context, id string) (*db_model.PromotionRule, error) {
	...
	err := s.db.Transaction(func(tx *gorm.DB) error {
		...
		rule, err = s.promotionRepository.Delete(ctx, tx, id)
		if err != nil {
			return err
		}
...
func (s *Promotion) ListRulesBySource(ctx context.Context, sourceCompetitionID string) ([]*db_model.PromotionRule, error) {
	return s.promotionRepository.ListBySourceCompetitionID(ctx, s.db, sourceCompetitionID)
}

// GetPromotionStatus は進出先の期待チーム数と現在のエントリー数を返す。
func (s *Promotion) GetPromotionStatus(ctx context.Context, targetCompetitionID string) (*model.PromotionStatus, error) {
...
func (s *Promotion) TryPromoteFromLeague(ctx context.Context, tx *gorm.DB, sourceCompetitionID string) error {
	// 1. 進出ルールを取得
	rules, err := s.promotionRepository.ListBySourceCompetitionID(ctx, tx, sourceCompetitionID)
	if err != nil {
...
func (s *Promotion) processOneRule(ctx context.Context, tx *gorm.DB, rule *db_model.PromotionRule, standings []*model.Standing) error {
...
func (s *Promotion) calcExpectedTeamCount(ctx context.Context, tx *gorm.DB, targetCompetitionID string) (int, error) {
	rules, err := s.promotionRepository.ListByTargetCompetitionID(ctx, tx, targetCompetitionID)
	if err != nil {
...
func (s *Promotion) checkTargetNotScored(ctx context.Context, tx *gorm.DB, targetCompetitionID string) error {
...
// NOTE: DB設計上 score=0 と「未入力」は区別できないため、status ベースで判定に統一する。
func (s *Promotion) CheckScoreModificationAllowed(ctx context.Context, tx *gorm.DB, sourceCompetitionID string) error {
	rules, err := s.promotionRepository.ListBySourceCompetitionID(ctx, tx, sourceCompetitionID)
	if err != nil {
		return errors.Wrap(err)
	}
...
func (s *Promotion) ReconstructPromotions(ctx context.Context, tx *gorm.DB, sourceCompetitionID string) error {
	rules, err := s.promotionRepository.ListBySourceCompetitionID(ctx, tx, sourceCompetitionID)
	if err != nil {
		return errors.Wrap(err)
	}
...
func (s *Promotion) computeStandingsForPromotion(ctx context.Context, tx *gorm.DB, leagueID string) ([]*model.Standing, error) {
...
func (s *Promotion) IsAllMatchesComplete(ctx context.Context, tx *gorm.DB, competitionID string) (bool, error) {
