...
func (r user) BatchCreate(ctx context.Context, db *gorm.DB, users []*db_model.User) ([]*db_model.User, error) {
	if len(users) == 0 {
		return users, nil
	}
...
func (r user) Delete(ctx context.Context, db *gorm.DB, id string) error {
	if err := db.WithContext(ctx).Delete(&db_model.User{}, "id = ?", id).Error; err != nil {
...
func (r user) FindUserIdpBySub(ctx context.Context, db *gorm.DB, sub string) (*db_model.UsersIdp, error) {
	...
	if err := db.WithContext(ctx).First(&record, "sub = ?", sub).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, errors.ErrUserNotFound
		}
		return nil, errors.Wrap(err)
...
func (r user) FindUserIdpByMicrosoftUserID(ctx context.Context, db *gorm.DB, microsoftUserID string) (*db_model.UsersIdp, error) {
	var record db_model.UsersIdp
	if err := db.WithContext(ctx).First(&record, "microsoft_user_id = ?", microsoftUserID).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, errors.ErrUserNotFound
		}
		return nil, errors.Wrap(err)
...
func (r user) BatchFindUserIdpByMicrosoftUserIDs(ctx context.Context, db *gorm.DB, microsoftUserIDs []string) ([]*db_model.UsersIdp, error) {
	if len(microsoftUserIDs) == 0 {
		return nil, nil
	}
	var records []*db_model.UsersIdp
	if err := db.WithContext(ctx).Where("microsoft_user_id IN (?)", microsoftUserIDs).Find(&records).Error; err != nil {
		return nil, errors.Wrap(err)
	}
	return records, nil
}
...
func (r user) BatchFindUserIdpByUserIDs(ctx context.Context, db *gorm.DB, userIDs []string) ([]*db_model.UsersIdp, error) {
	var records []*db_model.UsersIdp
	if err := db.WithContext(ctx).Where("user_id IN (?)", userIDs).Find(&records).Error; err != nil {
		return nil, errors.Wrap(err)
	}
	return records, nil
}
...
func (r user) SaveUserIdp(ctx context.Context, db *gorm.DB, userIdp *db_model.UsersIdp) (*db_model.UsersIdp, error) {
	if err := db.WithContext(ctx).Where("user_id = ?", userIdp.UserID).Save(userIdp).Error; err != nil {
		return nil, errors.Wrap(err)
	}
	return userIdp, nil
}

func (r user) BatchCreateUserIdp(ctx context.Context, db *gorm.DB, userIdps []*db_model.UsersIdp) ([]*db_model.UsersIdp, error) {
	if len(userIdps) == 0 {
		return userIdps, nil
	}
	if err := db.WithContext(ctx).Create(&userIdps).Error; err != nil {
		return nil, errors.Wrap(err)
	}
	return userIdps, nil
}
...
func (r user) GetRoleByUserID(ctx context.Context, db *gorm.DB, userID string) (*db_model.UserRole, error) {
	...
	if err := db.WithContext(ctx).First(&record, "user_id = ?", userID).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
