...
export function ClassCreatePage({ onBack }: Props) {
  ...
  return (
    <Box>
      ...
      <Breadcrumbs separator="/" sx={{ mb: 2 }}>
        ...
        <Typography sx={BREADCRUMB_CURRENT_SX}>クラス作成</Typography>
      </Breadcrumbs>
      ...
      <Box sx={{ background: CARD_GRADIENT, borderRadius: 2, p: 2 }}>
        ...
        <TextField fullWidth size="small" label="クラス名*" value={name} onChange={(e) => setName(e.target.value)} error={submitted && !name.trim()} helperText={submitted && !name.trim() ? 'この項目は必須です' : name.length >= 60 ? `${name.length}/64文字` : ''} sx={INPUT_SX} slotProps={{ htmlInput: { maxLength: 64 } }} />
